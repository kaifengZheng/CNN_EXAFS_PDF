...
config=toml.load("config_wr.toml")
...
def write_FEFFinp(template_dir,pos_filename,CA,site,radius,numbers=1):
    ...
    if "_site" not in str_title:
        ...
        title=pos_filename.split('.')[0].split('/')[-1]+f"_n_{numbers}"
    #print(radius)
...
def calc_pot_atoms_list(path, absorber = None, radius = 8, absorber_list = []):
    ...
    ISunique_absorber=len(absorber_list)
    #print(absorber_list)
    for i in absorber_list:
        ...
        #print(pot.struct.composition.items())
        for el, amt in pot.struct.composition.items():
            #print(amt)
            if ISunique_absorber==1:
                if el.symbol==absorber_species.symbol:
                    continue
                ipot = pot.pot_dict[el.symbol]
                ipotrow.append([ipot-1, el.Z, el.symbol, -1, -1, amt, 0])
            else:
                ipot = pot.pot_dict[el.symbol]
        ...
        for i in range(len(cluster)):
            if ISunique_absorber==1:
                if int(cluster[i][3])==0:
                    continue
                cluster[i][3]=str(int(cluster[i][3])-1)
    ...
    def particle_run(self):
        ...
        if os.path.exists(f'{run_dir}/xmu.dat'):
            ...
            js = {
                ...
                'k':     [str(np.float32(i.split()[2])) for i in xmudat[endhindex+1:]],
    ...
    if args.write_file==True:
        ...
        for i in tqdm(range(len(readfiles))):
            if symmetry:
                ...
                FEFF_obj.append(FEFF_cal(template_dir,readfiles[i],scratch,CA,radius,site=site,numbers=1))
